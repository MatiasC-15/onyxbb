...
globalThis.db = new Low(new JSONFile(`storage/databases/database.json`));
...
globalThis.authFile = `sessions`;
const { state, saveState, saveCreds } = await useMultiFileAuthState(`${globalThis.authFile}`);
...
async function connectionUpdate(update) {
  ...
  if (reason == 405) {
    await fs.unlinkSync(`${globalThis.authFile}/creds.json`);
